...
import {IOrderDataCenterQuery, IOrderDataCenterResponse, IOrderDataResponse} from "../interface";
...
export default function DataOrderContainer(){
    ...
    const [response,setResponse] = useState<IOrderDataCenterResponse|null>(null)
        ...
        <div style={{
            ...
            ,margin:'100px auto',
                ...
                <div style={{width:'100%',borderBottom:`3px solid ${amber[800]}`,paddingBottom:10}}>
                    <label style={{marginLeft:8}} htmlFor={'anchor'}>Móc thời gian</label>
                    <Input onChange={(e)=>handleDateChange(e)} value={anchor.toISOString().split('T')[0]} style={{marginLeft:8}} type={'date'} id={'anchor'}/>
                    <label style={{marginLeft:28}} htmlFor={'frame'}>Khung thời gian</label>
                    <Select onChange={(e)=>setConfigSelect(e.target.value)} value={configSelect} style={{marginLeft:8}}>
                        {
                            configData.map((config)=>{
                                return <MenuItem key={config.id} value={config.value}>{config.name}</MenuItem>
                            })
                        }
                    </Select>
                    <Button onClick={e=>handleSubmit()} style={{marginLeft:8,background:amber[800]}} variant={"contained"}>Dữ liệu</Button>
                </div>
                {/*card*/}
